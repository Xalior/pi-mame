...
PLATFORM_INTRO = {
    ...
    "sega": (
        "Sega's arcade racing hardware: the coin-operated board behind Out "
        "Run and the games Sega built on the same design (`segaorun.cpp` in "
        ...
        "FM sound chip for the music. Some of these machines use an "
        "encrypted processor, and where they do, the decryption key is a "
        "required part of the romset — the machine's own page says so. "
        "Sega's directory in MAME covers a great deal of other hardware as "
        "well; the machines listed below are the ones brought over so far. "
        "Each `make kernel MACHINE=<name>` below bakes one machine into its "
        "own `kernel8-<name>.img` — see the "
        "[top-level README](../../README.md) for the build and the regional "
        "canvas."
    ),
}

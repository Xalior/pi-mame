...
// nothing and MAME boots its own system-selection list. This kernel's own
// switches are consumed here (they set the flags below and never reach
// MAME), and so is every `--rapi-*` token: that namespace is
// circle-libsdl2's, which reads the same block for itself. Everything else
// passes through to MAME's argv, where MAME's own CLI frontend does all
// parsing.
//
...
// block's tokens minus the consumed switches. ppArgv must hold nMax
// pointers; the returned argc never exceeds nMax. Token storage is
...
// Kernel flags set by consumed switches. Plain ints with C

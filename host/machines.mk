...
#                                 carries its raster as --rapi-vfb=WxH —
#                                 the virtual display MAME renders 1:1, which
#                                 the shim's presentation core scales to the
#                                 glass; absent, the kernel declares the
#                                 720x576 PAL region canvas). Full-line #
#                                 comments and blank lines are dropped, the
#                                 remaining tokens join into the one line the
#                                 0x800 block carries — read it through
#                                 `print-string-<machine>`, below
...
MACHINE_ASSETS_bbcbp128     = bbcbp saa5050
MACHINE_ASSETS_ltmpbp       = bbcbp saa5050
MACHINE_ASSETS_bbcm         = bbcm saa5050
MACHINE_ASSETS_bbcmt        = bbcm saa5050
MACHINE_ASSETS_bbcmet       = bbcmet saa5050
MACHINE_ASSETS_bbcm512      = bbcm saa5050
MACHINE_ASSETS_ltmpm        = bbcm saa5050
...
MACHINE_ASSETS_mtx500       = mtx512
MACHINE_ASSETS_rs128        = mtx512
...
MACHINE_ASSETS_laser200     = laser110
MACHINE_ASSETS_fellow       = laser110
MACHINE_ASSETS_tx8000       = laser110
MACHINE_ASSETS_laser210     = laser210
MACHINE_ASSETS_vz200        = vz200
MACHINE_ASSETS_laser310     = laser310
MACHINE_ASSETS_vz300        = laser310
MACHINE_ASSETS_laser310h    = laser310
...
MACHINE_ASSETS_crvisioj     = crvision
MACHINE_ASSETS_wizzard      = crvision
MACHINE_ASSETS_rameses      = fnvision
MACHINE_ASSETS_vz2000       = fnvision
MACHINE_ASSETS_crvisio2     = crvision
...
MACHINE_ASSETS_sharrier     = sharrier
MACHINE_ASSETS_enduror      = enduror

# Query helpers. Lets scripts read these facts without pulling in the
# Circle build.
#
#   make -f machines.mk -s print-MACHINE_ASSETS_spectrum   a variable
#   make -f machines.mk -s print-string-spectrum           the defaults line
#
# print-string-% reads machines/<machine>.conf: full-line # comments and
# blank lines are dropped, and the remaining tokens join into the ONE line
# the 0x800 defaults block carries. A missing conf file is a loud error —
# that is the "unknown machine" gate.
MACHINES_CONF_DIR := $(patsubst %/,%,$(dir $(lastword $(filter %machines.mk,$(MAKEFILE_LIST)))))/machines

print-string-%:
	@[ -f '$(MACHINES_CONF_DIR)/$*.conf' ] || \
		{ echo "unknown machine '$*' — no $(MACHINES_CONF_DIR)/$*.conf" >&2; exit 1; }
	@awk '!/^[ \t]*(\#|$$)/ { for (i = 1; i <= NF; i++) printf "%s%s", (n++ ? " " : ""), $$i } END { print "" }' \
		'$(MACHINES_CONF_DIR)/$*.conf'

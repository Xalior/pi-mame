...
![Commodore 128DCR (NTSC) at power-on](images/c128dcr.jpg)

- **`make MACHINE=c128dcr`** — Commodore Business Machines
- **Year**: 1987
...
parts were consolidated into fewer, larger combined ROMs. In MAME this NTSC unit
(`c128dcr`, 1987) is a clone of the base `c128` in the
...
its **own `c128dcr` machine config**, which builds on the same NTSC base as the
plain 128 — so what you get is the full C128 on a cost-reduced board.
...
This is the **NTSC** machine — its `c128dcr` config calls the NTSC base — and it
...
- `roms/c128dcr.zip`
  ...
  Like the `c128cr` (and unlike the aliased `c128d` / `c128dp`), c128dcr is
  **not** a romset alias — it has its own `ROM_START( c128dcr )`. The cost-reduced
  board's two **combined ROMs** (`318022-02`, `318023-02`) are **unique to
  c128dcr** and come from the split-set `c128dcr.zip`; the **character generator**
  (`390059-01`) and the **PLA** (`8721r3.u11`) are byte-for-byte the c128 line's,
  located by checksum in the family parent `c128.zip` and repacked.
...
- **A cost-reduced 128D — an NTSC 128 in MAME's model.** The 128DCR simplified
  the board and merged the ROMs. In MAME (`c128dcr`) it is a clone of `c128`
  driven by its own `c128dcr` machine config, which is functionally the NTSC 128;
  only the (unique, combined) ROM layout differs.
- **Two combined ROMs, not aliased.** Where `c128d`/`c128dp` reuse the parent's
  six-member set verbatim, c128dcr consolidates BASIC/editor/kernal into
  `318022-02` + `318023-02` — its own romset. It shares only the chargen and PLA
  with the c128 line.
- **A dual-CPU machine.** The C128 carries a **Z80** (for CP/M mode) *and* an
  **8502** (for 128 and C64 modes). They share one memory map and one kernal ROM
  complement, so there is no separate Z80 BIOS romset — the single `c128dcr.zip`
...
- **The IEC disk bus boots empty.** The real 128DCR has a built-in 1571CR drive,
  but MAME 0.250's `c128dcr` config does **not** model it as built-in: it defaults
  a stock **C1571** at device 8 on the external serial bus
  ...
  `-iec8 ""`, exactly as `c128d`/`c128cr` do; a 128 with nothing answering on its
  serial port is a completely valid configuration for reaching BASIC. No
  drive-device romset is required or shipped.

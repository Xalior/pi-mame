...
![Commodore 128D (PAL) at power-on](images/c128dp.jpg)

- **`make MACHINE=c128dp`** — Commodore Business Machines
- **Year**: 1986
...
with a detachable keyboard. This is the **PAL** unit (`c128dp`, 1986), a clone
of the base `c128` in the `src/mame/commodore/c128.cpp` driver family
(`c128_state`), distinct from the `c64.cpp`, `vic20.cpp` and `plus4.cpp` lines
already on this appliance. MAME drives this PAL 128D with the **`c128pal`
machine config** — the same PAL config the `c128p` root uses — so what you get
is the full C128 in a desktop shell, timed for PAL.
...
sibling `c128d` uses the base `c128` config) — and it fills the **720x576 PAL
...
- `roms/c128dp.zip`
  ...
  c128dp **shares the c128 family parent's romset** — the driver aliases it
  (`#define rom_c128dp rom_c128`), so the members are byte-for-byte the c128 set;
...
- **A desktop 128 — a PAL 128 in MAME's model.** The 128D packaged a C128 and a
  C1571 drive in one case. In MAME the PAL unit (`c128dp`) is a clone of `c128`
  driven by the `c128pal` machine config, so it is functionally the PAL 128; the
  same romset boots it.
- **A dual-CPU machine.** The C128 carries a **Z80** (for CP/M mode) *and* an
  **8502** (for 128 and C64 modes). They share one memory map and one kernal ROM
  complement, so there is no separate Z80 BIOS romset — the single `c128dp.zip`
